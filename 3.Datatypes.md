...
null == undefined; // true
```
>The string "2" is converted to a number before being compared to the number 2. In the second example, 
...
```
>The string "2" and the number 2 are not of the same type, so the comparison returns false. In the second example, 
